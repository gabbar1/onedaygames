...
class TicketScreen extends StatefulWidget {
  ...
  TicketScreen({this.phone,this.price,this.amt,this.typ,this.ticket_type,this.numberofsell,this.total_amount,this.winning_amount,this.added_amount,this.ticket_id,this.deadline,this.email,this.index1});
...
class _TicketScreenState extends State<TicketScreen> {
  ...
  cutAmount() async {
    ...
    int  amount = (await FirebaseDatabase.instance.reference().child("Wallet").child(widget.phone).child("amount").once()).value;
    if(amount!=null) {
                      ...
                      onPressed: (){
                        ...
                        if (strPin == null ||strPin.length < 4){
                          ...
                          return showDialog<void>(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  title: Text(confirm,style:GoogleFonts.barlowCondensed(textStyle: Theme.of(context).textTheme.headline5,fontSize: 15,fontWeight: FontWeight.bold)),
                                  content: SingleChildScrollView(
                                    child: ListBody(
                                      children: <Widget>[
                                        Text(ticket_amount+widget.amt+" ₹",style:GoogleFonts.barlowCondensed(textStyle: Theme.of(context).textTheme.headline5,fontSize: 15,fontWeight: FontWeight.bold)),
                                  ...
                                  actions: <Widget>[
                                    RaisedButton(
                                      ...
                                      onPressed: () async {
                                          ...
                                          DBRef.child("Ready_for_Result").child(widget.ticket_type).child(widget.ticket_id).child(widget.phone).child((number+1).toString()).set({
                                            'name':widget.typ,
                                            'phone':widget.phone,
                                            'price':widget.price,
                                            'amount':widget.amt,
                                          ...
                                          DBRef.child("Lottery").child(widget.ticket_type).child(widget.ticket_id).update({
                                            'numberofsell':int.parse(widget.numberofsell)+1
                                          });
                                          if(widget.added_amount==0 ||widget.added_amount == null){

                                            DBRef.child("Wallet").child(widget.phone).update({
                                              'winning_amount':widget.winning_amount-int.parse(widget.amt),
                                            ...
                                            DBRef.child("Wallet").child(widget.phone).update({
                                              'added_amount':widget.added_amount-int.parse(widget.amt),
                                              'total_amount':widget.total_amount-int.parse(widget.amt),
                                              'time':DateFormat('EEEE, d MMM, yyyy,h:mm:ss a').format(now),
                                            });
                                          }
                                          transRef.child('User_Transaction').child(widget.phone).push().set({
                                            'amount':widget.amt,
                                            'status':"T",
                                            'time':DateFormat('EEEE, d MMM, yyyy,h:mm:ss a').format(now),
                                          });
                                          Fluttertoast.showToast(msg: number.toString());
                                          transRef.child("Bought_tickets").child(widget.phone).child(widget.ticket_type).child(widget.ticket_id).update({
                                            'number':number+1
                                          });
                                          transRef.child("Users").child(widget.phone).update({
                                    ...
                                    RaisedButton(
                                      color: Colors.red,
                                      child: Text(cancel,style:GoogleFonts.barlowCondensed(textStyle: Theme.of(context).textTheme.headline5,fontSize: 15,fontWeight: FontWeight.bold)),
                                      onPressed: () {
                                        Navigator.of(context).pop();
                                      },
                                    ),
  ...
  buildSecurityText(){
    return
      Column(children: [
        ListTile(title: Center(child: Text(ticket_amount1+widget.amt+ " ₹",style:GoogleFonts.barlowCondensed(textStyle: Theme.of(context).textTheme.headline5,fontSize: 15,fontWeight: FontWeight.bold,color: Colors.red)),)),
        ListTile(title: Center(child: Text(luck_number,style:GoogleFonts.barlowCondensed(textStyle: Theme.of(context).textTheme.headline5,fontSize: 15,fontWeight: FontWeight.bold,color: Colors.white70),)))
      ],);
